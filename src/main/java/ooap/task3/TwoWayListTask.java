...
import ooap.task2.LinkedList;

public class TwoWayListTask {
    interface ParentList<T> {
        // Команды
    ...
    interface LinkedList<T> extends ParentList<T> {}
    interface TwoWayList<T> extends ParentList<T> {
        // Команды

        // Предусловия: список не пустой, текущий указатель находится не в голове
        // Постусловия: указатель сдвинут на один элемент вправо
        void left();

        // Запросы

        // Постусловия: возвращен статус последнего вызова метода left()
        LeftStatus getLeftStatus();

        enum LeftStatus { NOT_INVOKED, LIST_EMPTY, ITERATOR_ON_HEAD, ITERATOR_NOT_SET }
    }
